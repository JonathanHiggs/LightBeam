...
namespace LightBeam
{
	namespace Shapes
	{
			...
			HitRecord& record) const
		{
			...
			const auto uv = uv_from_point(point, ray.time());
		...
		Vec2 MovingSphere::uv_from_point(const Math::Vec3& point, double time) const
		{
			auto relative_point = (point - center(time)) / _radius;
